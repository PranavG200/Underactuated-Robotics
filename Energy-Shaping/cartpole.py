...
class Cartpole(object):
  ...
  def compute_energy_shaping_input(self, t, x):
    ...
    Ehat = 0.5*qdot[1]**2 - (self.g)*cos(q[1]) - self.g 
    q1_ddot_des = qdot[1]*cos(q[1])*Ehat
    ...
    u = (self.mc +self.mp)*q1_ddot_des - self.mp*cos(q[1])*(self.g *sin(q[1]) + cos(q[1])*q1_ddot_des) - self.mp*self.L*sin(q[1])*(qdot[1]**2)
