    ...
    for s in range(self.rows * self.cols):
      ...
      # STUDENT CODE HERE
            ...
            P[s][1] = [(slip_prob, s, large_cost),
                       (1-slip_prob, s+1, large_cost)]
            P[s][3] = [(slip_prob, s, large_cost),
                       (1-slip_prob, s-1, large_cost)]
            P[s][0] = [(slip_prob, s, large_cost),
                       (1-slip_prob, s - self.rows, large_cost)]
            P[s][2] = [(slip_prob, s, large_cost),
                       (1-slip_prob, s + self.rows, large_cost)]
        else:
            P[s][1] = [(slip_prob, s, base_cost),
                       (1-slip_prob, s+1, base_cost)]
            P[s][3] = [(slip_prob, s, base_cost),
                       (1-slip_prob, s-1, base_cost)]
            P[s][0] = [(slip_prob, s, base_cost),
                       (1-slip_prob, s - self.rows, base_cost)]
            P[s][2] = [(slip_prob, s, base_cost),
                       (1-slip_prob, s + self.rows, base_cost)]

        if (r == 0):
            P[s][0] = [(slip_prob,s,base_cost),
                        (1 -slip_prob,s, wall_cost)]
            if (c == 0):
                P[s][3] = [(slip_prob,s, base_cost),
                        (1-slip_prob,s, wall_cost)]
            if (c == 9):
