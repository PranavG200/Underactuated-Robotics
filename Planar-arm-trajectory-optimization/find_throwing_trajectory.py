...
def find_throwing_trajectory(N, initial_state, final_configuration, distance, tf):
  ...
  uGuess= np.zeros((N,n_u))
  for i in range(N):
      uGuess[i,:] = np.random.uniform(low=-effort_limits, high=effort_limits, size=(n_u))
  prog.SetInitialGuess(u,uGuess)

  xGuess=np.ones((N,n_x))
  prog.SetInitialGuess(x,xGuess)
